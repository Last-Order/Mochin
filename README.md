...
工程使用根目录的 `partitions.csv` 管理 16 MB Flash，当前不启用 OTA。
主要分区如下：

| 分区 | 大小 | 用途 |
| --- | ---: | --- |
| `nvs` | 24 KiB | 保存设置、Wi-Fi 配置和宠物状态 |
| `phy_init` | 4 KiB | 射频初始化数据 |
| `factory` | 8 MiB | 应用、LVGL 字体和初期内嵌图片 |
| `coredump` | 256 KiB | 后续启用崩溃转储时保存现场 |

`0x850000` 之后约 7.69 MiB 暂不分区，留给后续确定格式的 `assets`
文件系统。扩大应用分区只提高固件大小上限，不会让实际生成的固件或每次
需要写入的数据自动增长。
